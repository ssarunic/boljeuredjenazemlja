    ...
    try:
        with CadastralAPIClient() as client:
            ...
            if output_format != "table":
                ...
                # Rich table output
                _print_table_output(lr_unit, show_owners, show_parcels, show_encumbrances, show_all)
...
def _print_summary(lr_unit) -> None:
...
def _print_ownership_sheet(lr_unit) -> None:
...
def _print_parcel_list(lr_unit) -> None:
...
def _print_encumbrance_sheet(lr_unit) -> None:
    ...
    for group in lr_unit.encumbrance_sheet_c.lr_entry_groups:
        entries_text = "\n".join(
            f"• {entry.order_number}: {_clean_html(entry.description)[:150]}..."
